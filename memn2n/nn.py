...
class CrossEntropyLoss(Loss):
    ...
    def get_precision_recall_yes(self, input_data, target_data):
        y = input_data.argmax(axis=0)
        tp = np.sum(np.logical_and(y == 1, target_data == 1))
        fp = np.sum(np.logical_and(y == 1, target_data == 2))
        fn = np.sum(np.logical_and(y == 2, target_data == 1))

        return tp / (tp + fp), tp / (tp + fn), tp, fp, fn


    def get_precision_recall_no(self, input_data, target_data):
        y = input_data.argmax(axis=0)
        tp = np.sum(np.logical_and(y == 2, target_data == 2))
        fp = np.sum(np.logical_and(y == 2, target_data == 1))
        fn = np.sum(np.logical_and(y == 1, target_data == 2))
        return tp / (tp + fp), tp / (tp + fn), tp, fp, fn
